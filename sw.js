...
// 📷 지연 로딩 모듈(vision.js · scanner.js · vendor/quagga2.min.js)은 **일부러 PRECACHE 에 안 넣는다.**
// 넣으면 그 기능을 한 번도 안 쓸 사람에게까지 첫 방문에 받게 한다 —
// "안 쓰는 사람에게 비용 0" 이 두 기능의 공통 설계 전제다(리서치/바코드-매장스캔-조사.md 8-D).
// ⚠️ vision.js 는 외부 라이브러리가 없어 훨씬 가볍다(수십 KB). 그래도 같은 이유로 안 넣는다 —
//    사진 인식은 워커에 키가 꽂혀 있을 때만 켜지므로, 안 켜진 배포에선 아예 쓸 일이 없다.
