...
window.__fuzz = (function () {
  ...
  // ⭐ 우리가 인용하는 기본 포털은 Rakuten·TopCashback 둘뿐이고,
  //    **CapOne 은 요율 비교에서 행째로 빠졌다**(v0.36).
  //
  // CapOne 은 CAPTCHA 로 자동 수집이 안 돼 수동 스냅샷이었고, 그 값이 3주씩 묵은 채
  // 어제 값과 동등한 무게로 순위에 들어가고 있었다.
  //
  // ⚠️ 중간에 **'모름'(pct:null → "직접 확인")으로 남기던 시기가 있었고 이 검사도 그걸 봤다.**
  //    지금은 **키 자체가 없는 것**이 정답이다 — '모름'을 남긴 이유가 *"넣을 방법을
  //    발견하게 하려고"* 였는데 넣는 칸을 안 만들기로 했고, 사용자가 할 수 있는 게 없는
  //    `?` 는 노이즈이기 때문이다.
  //    **정의가 바뀌었는데 검사가 옛 정의를 보고 있으면 그때부터 검사가 거짓말을 한다.**
  //    그래서 판정 기준을 같이 옮겼다.
  //
  // 이 검사가 지키는 것 두 가지:
  //   ① CapOne 이 요율 비교에 **다시 들어오지 않는다** (`'cap' in b` 가 false)
  ...
  function runPortalDefault() {
    ...
    for (const k of withCap.slice(0, 12)) {
      ...
      checks++;
      // ★ 행 자체가 없어야 한다.
      if ('cap' in b) bad.push('★ CapOne 이 아직 요율 비교에 들어 있다 (키 자체가 없어야 한다): ' + k + ' → ' + JSON.stringify(b.cap));
      checks++;
      // ★ 뺄 때 0 으로 뭉개면 안 된다 — 없는 것과 0% 는 다르고,
